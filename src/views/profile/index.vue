  ...
  <div class="profile-container">
    <el-row class="header">
      <el-col :sm="24" :lg="12" class="header-left">
        ...
        <div class="header-content">
          <div class="title">
            {{ username }} 您好，您有 <span>2</span> 条未读消息请尽快查阅。
          </div>
          <div class="sub">
            职位：前端工程师
          </div>
          <div class="route">当前权限：{{ about }}</div>
          <div class="sub">最后登录时间：{{ lastLoginDate }}</div>
        </div>
      </el-col>
    ...
    <el-row class="profile-content" :gutter="20" >
      <el-col :sm="24" :lg="18" class="profile-content__left">
        <el-alert
          title="提示：您在 2019/09/23 生成的系统报告已提交成功"
          type="success"
        >
        </el-alert>
        <el-alert title="警告：目前系统有三个资源值请求数量过大" type="warning">
        </el-alert>
        <el-card shadow="hover">
          ...
          <div class="feed" v-for="(item, index) in feedmock" :key="index">
            <div class="line">
              ...
              <el-button class="details" type="primary" size="mini" plain
                >详情</el-button
              >
            </div>
          </div>
...
const feedmock = [
  {
    content: '审核vue-admin-tempalate 项目并通过',
    time: '一小时前'
  },
  {
    content: '开始监控 sakuya，pipi 和另外两个用户',
    time: '五小时前'
  }
]
...
export default {
  ...
  data() {
    return {
      ...
      about: this.$store.state.user.introduction,
